...
function RoomCard({ room, onJoin }) {
    return (
        <div
            className="w-[75%] h-[90%] flex-shrink-0 relative group perspective cursor-pointer"
            onClick={onJoin}
        >
            ...
            <div className="relative z-10 p-6 flex flex-col h-full justify-between">
                <div className="flex justify-between items-start">
                    <div>
                        <span className="block text-2xl font-black text-slate-800">
                            {new Date(room.departure_time).getDate()}일
                        </span>
                    ...
                    <span className="bg-orange-500/10 text-orange-600 text-xs font-extrabold px-3 py-1.5 rounded-full border border-orange-500/20 shadow-sm backdrop-blur-sm">
                        모집중
                    </span>
                </div>
                ...
                <button className="w-full py-3 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm hover:bg-slate-200 transition-colors">
                    채팅방 입장
                </button>
            </div>
        </div>
